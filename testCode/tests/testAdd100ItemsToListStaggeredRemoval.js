...
module.exports = {
  ...
  '7. Remove a specific item in the list': function(client) {
    this.theWebpage.removeItem('58', function(foundIndex){
      this.currentList.splice(foundIndex,1);
    }.bind(this));
  },

  '8. Get count of items left': function(client){
    this.theWebpage.getCountOfItemsLeft(function(returnItemsLeft){
      client.verify.equal(returnItemsLeft, "99 items left", "Verifying the number of items not completed.");
    });
  },

  '9. Verify only the one item was removed': function(client){
    this.theWebpage.getTodoList(function(returnList){
      client.verify.deepEqual(returnList,this.currentList, "Making sure the item got removed from the list.");
    }.bind(this));
  },

  '10. Mark item as completed in todo list': function(client){
    let markCompletedItemList = ["45","8","0","99","23","10"];
    for (i = 0; i < markCompletedItemList.length; i++) {
      this.theWebpage.toggleItemCompleted(markCompletedItemList[i], function(foundIndex){
        this.currentList[foundIndex][1] = true;
      }.bind(this));
    }
  },

  '11. Get count of items left': function(client){
    this.theWebpage.getCountOfItemsLeft(function(returnItemsLeft){
      client.verify.equal(returnItemsLeft, "93 items left", "Verifying the number of items not completed.");
    });
  },

  '12. Verify only the marked items have changed': function(client){
      this.theWebpage.getTodoList(function(returnList){
        client.verify.deepEqual(returnList,this.currentList, "Making sure the items got marked as completed.");
      }.bind(this));
  },

  '13. Toggle the select All button': function(client){
  ...
  '17. Verify completed task items are removed': function(client){
    this.theWebpage.getTodoList(function(returnList){
      client.verify.deepEqual(returnList,this.currentList, "Making sure the item got removed from the list.");
    }.bind(this));
  },

  '18. Get Performance Logs': function (client) {
