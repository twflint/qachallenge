module.exports = {
  ...
  '6. Remove a specific item in the list': function(client) {
    this.theWebpage.removeItem('Goto Store', function(foundIndex){
  ...
  '7. Get count of items left': function(client){
    this.theWebpage.getCountOfItemsLeft(function(returnItemsLeft){
      client.verify.equal(returnItemsLeft, "", "Verifying the number of items not completed.");
    });
  },

  '8. Verify only the one item was removed': function(client){
    this.theWebpage.getTodoList(function(returnList){
      client.verify.deepEqual(returnList,this.currentList, "Making sure the item got removed from the list.");
    }.bind(this));
  },

  '9. Get Performance Logs': function (client) {
